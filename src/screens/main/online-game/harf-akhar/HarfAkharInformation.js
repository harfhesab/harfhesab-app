...
import { createHarfAkharChallenge } from '../../../../realm/repositories/harf-akhar/harf-akhar-challenge.repository';
...
function HarfAkharInformation(props){
    ...
    const cardImageBackground = data?.subscription_required == true?
        ...
        require("../../../../assets/image/harf-akhar-card-1.png")
    ...
    const getData = async()=>{
        await axios({
            ...
            data: {
                query : `
                    query getHarfAkharChallengeInformationAndUserStatus(
                        $_id : ID!
                    ){
                        getHarfAkharChallengeInformationAndUserStatus(
                            _id : $_id
                        ) {
            ...
            const dataReceived = response.data.data?.getHarfAkharChallengeInformationAndUserStatus
            if(dataReceived){
                setData(dataReceived?.harf_akhar_challenge)
                if(dataReceived?.user_progress)setProgress(dataReceived?.user_progress)
                setLoading(false)
            }
            const endDate = dataReceived?.harf_akhar_challenge?.end_date
    ...
    const startChallengeCheck = ()=>{
        ...
        } else if(!progress && data?.subscription_required == true && activeSubscription !== true){
            const btn = [
                {
                    onPress : ()=>{
        ...
        } else if(data?.entry_fee_coins && data?.entry_fee_coins > numberCoins){
                ...
                buttons:[
                    {
                        onPress : ({data})=>{
                            if(data._id[0] == "1"){
                                ...
                                props.navigation.navigate("FreeCoin")
                            }
                        },
                        text: "افزایش سکه",
                        loading: false,
    ...
    const startChallenge = async()=>{
        ...
        await axios({
            ...
            data: {
                query : `
                mutation startNewSessionForHarfAkharChallenge(
                        $challenge: ID!
                        $session: ID
                        $user_number_coins: Int!
                    ){
                    startNewSessionForHarfAkharChallenge(
                        challenge : $challenge
                        session : $session
            ...
            const receivedData = response.data.data?.startNewSessionForHarfAkharChallenge
            if(receivedData?.status == 200){
                ...
                const documentData = {
                    ...data,
                    ...receivedData.harf_akhar_challenge
                }
                const res = createHarfAkharChallenge(
                    realm,
                    documentData,
                    expiration,
                            ...
                            >
                                <View style={{flexDirection:'column', alignItems:'center', justifyContent:'space-between', width:"100%", height:"100%"}}>
                                    ...
                                    <View style={{width:"100%", alignItems:'center', paddingTop:20, gap:5}}>
                                        <ImageBackground
                                            source={require("../../../../assets/image/title-harf-akhar.png")}
...
export default HarfAkharInformation;