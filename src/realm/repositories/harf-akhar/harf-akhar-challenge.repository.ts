...
export const createHarfAkharChallenge = (
  realm: Realm,
  data: Partial<HarfAkharChallenge> & { _id: BSON.ObjectId | string },
  expiration: number,
  ...
  try {
    ...
    const existingDoc = realm.objectForPrimaryKey<HarfAkharChallenge>(
      "HarfAkharChallenge",
      objectId
    );
...
export const removeExpiredHarfAkharChallenges = (
  realm: Realm
): number => {
  try {
    const now = new Date();
    const expiredChallenges = realm
      .objects("HarfAkharChallenge")
...
export const getHarfAkharChallengeById = (
  realm: Realm,
  challengeId: BSON.ObjectId | string
): HarfAkharChallenge | null => {
  try {
    const objectId = typeof challengeId === "string"? new BSON.ObjectId(challengeId): challengeId;

    return realm
      .objectForPrimaryKey<HarfAkharChallenge>("HarfAkharChallenge", objectId) || null;
  } catch (error) {
    return null;
  }
};
...
export const deleteHarfAkharChallengeById = (realm: Realm, id: BSON.ObjectId | string): boolean => {
  try {
    const objectId = typeof id === "string" ? new BSON.ObjectId(id) : id;
    const target = realm.objectForPrimaryKey("HarfAkharChallenge", objectId);
...
export const getAllHarfAkharChallengesPlaying = (
  realm: Realm,
): HarfAkharChallenge[] => {
  try {
    const challenges = realm.objects<HarfAkharChallenge>(HarfAkharChallenge.schema.name);
    return Array.from(challenges);
  } catch (error) {
    console.error("Error fetching HarfAkharChallenges:", error);
    return [];
  }
};
