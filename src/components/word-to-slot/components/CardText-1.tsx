...
function FloatingCard({ word, index, numberOfCards }: Props) {
  ...
  useEffect(() => {
    registerCard({
      ...
      isAssigned,
      cardSize,
      fontSize: textScale, // برای سازگاری با DragDropContext
    });
    return () => {};
  }, [id, registerCard]);
  ...
  const snapToSlot = () => {
    ...
    if (closestSlot !== -1 && maxOverlap > 0) {
      ...
      textScale.value = withSpring(
        Math.min(FONT_SIZE_SLOTTED_SCALED / FONT_SIZE_FLOATING_SCALED, 0.8),
      ...
      textScale.value = withSpring(1, SPRING_CONFIG_SOFT);
      const newPos = {
  ...
  const pan = Gesture.Pan()
    ...
    .onStart(() => {
      ...
      textScale.value = withSpring(
        Math.max(FONT_SIZE_DRAGGING_SCALED / FONT_SIZE_FLOATING_SCALED, 1),
        SPRING_CONFIG_SOFT
      );
  ...
  const textContainerStyle = useAnimatedStyle(() => ({
    ...
    width: cardSize.value * 0.8, // محدود کردن عرض متن به 80% عرض کارت
    maxWidth: cardSize.value * 0.8, // سقف برای جلوگیری از سرریز
    maxHeight: cardSize.value * 0.8, // سقف برای جلوگیری از سرریز در ارتفاع
  }));
    ...
    <GestureDetector gesture={pan}>
      <Animated.View style={[styles.card, cardStyle]}>
        <Animated.View style={[styles.textContainer, textContainerStyle]}>
          <Text style={[styles.text, { fontSize: FONT_SIZE_FLOATING * fontSizeScale }]}>{word}</Text>
        </Animated.View>
      </Animated.View>
    </GestureDetector>
  );
...
const styles = StyleSheet.create({
  ...
  text: {
    ...
    textAlign: 'center',
    flexWrap: 'wrap',
    width: '100%', // عرض متن به اندازه textContainer
  },
});
