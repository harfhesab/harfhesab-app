...
// تنظیمات انیمیشن
const SPRING_CONFIG_SOFT = { stiffness: 200, damping: 16, mass: 1.4, overshootClamping: false };
...
function FloatingCard({ word, index, numberOfCards }: Props) {
  ...
  const FONT_SIZE_FLOATING_SCALED = FONT_SIZE_FLOATING;
  ...
  const textScale = useSharedValue(1); // مقیاس پایه برای حالت شناور
  ...
  const snapToSlot = () => {
    ...
    if (closestSlot !== -1 && maxOverlap > 0) {
      ...
      cardSize.value = withSpring(CARD_SIZE_SLOTTED, SPRING_CONFIG_SOFT);
      const scale = FONT_SIZE_SLOTTED_SCALED / FONT_SIZE_FLOATING;
      textScale.value = Math.min(scale, 0.6); // تنظیم مستقیم
      console.log('snapToSlot textScale:', textScale.value, 'FONT_SIZE_SLOTTED_SCALED:', FONT_SIZE_SLOTTED_SCALED, 'FONT_SIZE_FLOATING:', FONT_SIZE_FLOATING, 'fontSizeScale:', fontSizeScale, 'word:', word);
      ...
      textScale.value = 1; // تنظیم مستقیم
      console.log('snapToSlot textScale:', textScale.value, 'word:', word);
      const newPos = {
  ...
  const pan = Gesture.Pan()
    ...
    .onStart(() => {
      ...
      textScale.value = Math.max(FONT_SIZE_DRAGGING_SCALED / FONT_SIZE_FLOATING_SCALED, 1); // تنظیم مستقیم
      console.log('pan textScale:', textScale.value, 'word:', word);
  ...
  const textContainerStyle = useAnimatedStyle(() => {
    let safeScale = textScale.value;
    if (isAssigned.value && !isDragging.value) {
      safeScale = Math.min(FONT_SIZE_SLOTTED_SCALED / FONT_SIZE_FLOATING, 0.6); // پیش‌فرض برای اسلات
    } else if (isDragging.value) {
      safeScale = Math.max(FONT_SIZE_DRAGGING_SCALED / FONT_SIZE_FLOATING_SCALED, 1);
    } else {
      safeScale = 1;
    }
    console.log('textContainerStyle rawScale:', textScale.value, 'safeScale:', safeScale, 'isAssigned:', isAssigned.value, 'isDragging:', isDragging.value, 'word:', word);
    return {
      transform: [{ scale: safeScale }],
      width: cardSize.value * 0.8,
      maxWidth: cardSize.value * 0.8,
      maxHeight: cardSize.value * 0.8,
    };
  });
    ...
    <GestureDetector gesture={pan}>
      <Animated.View style={[styles.card, cardStyle]}>
        <Animated.View style={[styles.textContainer, textContainerStyle]}>
          <Text style={[styles.text, { fontSize: FONT_SIZE_FLOATING }]}>{word}</Text>
        </Animated.View>
      </Animated.View>
    </GestureDetector>
  );
